  ...
  const [rows] = await pool.execute(
    `
      SELECT
        ...
        COUNT(customer_signatures.id) AS signature_count
      FROM bookings
      INNER JOIN customers ON customers.id = bookings.customer_id
      INNER JOIN vehicles ON vehicles.id = bookings.vehicle_id
      LEFT JOIN customer_signatures ON customer_signatures.booking_id = bookings.id
...
export default async function RepSignaturesPage() {
  await getRepUser();
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <Link href="/rep/dashboard" className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-xs text-white/50">Customer Signatures</p>
            </div>
          </Link>
      ...
      <section className="mx-auto max-w-7xl px-5 py-8">
        <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#0b1f3a] to-[#26070a] p-6 shadow-2xl md:p-10">
          ...
          <p className="mt-4 max-w-2xl text-white/70">
            Select a booking and let the customer sign the rental agreement
