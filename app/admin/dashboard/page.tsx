...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    db.query(
      `
        ...
        WHERE LOWER(status) IN ('pending', 'contacted', 'approved')
      `
    ),
    db.query(
      `
  ...
  const pendingRequests = getFirstTotal(requestRows);
  const recentBookings = recentRows as BookingRow[];

  return (
    <main className="min-h-screen bg-[#f7f3ea] text-[#1d1d1f]">
      <AdminMobileHeader />
      ...
      <div className="flex min-h-screen">
        ...
        <section className="flex-1">
          <header className="border-b border-black/10 bg-white px-6 py-6 md:px-8">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div>
                ...
                <h1 className="mt-2 font-serif text-4xl font-black text-[#1d1d1f] md:text-5xl">
                  Office Command Center
                </h1>

                <p className="mt-2 text-sm font-semibold text-[#6b6257]">
                  Manage bookings, requests, vehicles, customers, staff, and
                  today&apos;s rental activity from one dashboard.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <Link
                  ...
                  className="inline-flex items-center justify-center rounded-xl border border-[#d4af37]/40 bg-white px-6 py-4 text-sm font-black text-[#4b443d] shadow-sm transition hover:-translate-y-0.5"
                >
                  Website Requests
                  {pendingRequests > 0 ? (
                    <span className="ml-3 rounded-full bg-red-600 px-2 py-1 text-xs text-white">
                      {pendingRequests}
                    </span>
                  ) : null}
                </Link>
          ...
          <div className="space-y-6 p-5 md:p-8">
            <section className="overflow-hidden rounded-[2rem] border border-black/10 bg-black shadow-xl">
              <div className="relative min-h-[280px] overflow-hidden">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_25%,rgba(212,175,55,0.34),transparent_34%),linear-gradient(90deg,#040404_0%,#111111_48%,#42270e_100%)]" />
                ...
                <div className="relative grid min-h-[280px] gap-8 px-8 py-8 md:grid-cols-[1.15fr_0.85fr] md:items-center md:px-10">
                  <div className="max-w-2xl">
                    ...
                    <p className="mt-6 max-w-xl text-sm font-semibold leading-7 text-white/72">
                      Customer requests now flow into the office, and staff can
                      convert them into real bookings with vehicle reservation.
                    </p>
                  </div>
            ...
            <section className="grid gap-4 lg:grid-cols-4">
              ...
              <ActionPanel
                ...
                value={String(pendingRequests)}
                note="New customer requests waiting for office action."
                href="/admin/booking-requests"
              ...
              <div className="flex flex-col gap-4 border-b border-black/10 px-6 py-5 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold text-[#7a7168]">
                    Latest activity in the office system.
                  </p>
                </div>

                <Link
                  href="/admin/bookings"
                  className="rounded-xl border border-black/10 bg-white px-5 py-3 text-sm font-black text-[#4b443d] shadow-sm"
                >
                  View All Bookings
                </Link>
              </div>
...
function DashboardCard({
  ...
  return (
    <Link
      href={href}
      className={`rounded-[2rem] p-6 shadow-xl shadow-black/5 transition hover:-translate-y-1 hover:shadow-2xl ${styles[tone]}`}
    >
